...
import { EchoScanMaterial } from "./EchoScanMaterial";
...
void main() {
  vec2 coord = position.xy * 5.0 + vec2(0.0, time / 5000.0);
...
class EchoScanProgram implements Program {
  ...
  static create(
    gl: WebGL2RenderingContext
  ): EchoScanProgram | undefined {
    const glProgram = GLProgram.create(gl, vertexShaderSource, fragmentShaderSource, 'FallingLeavesProgram');
    if (!glProgram) {
      console.error('[ERROR] EchoScanProgram.create() could not create GLProgram');
      return undefined;
    }

    const glCamera = GLCamera.create(gl, glProgram.program);
    if (!glCamera) {
      console.error('[ERROR] EchoScanProgram.create() could not create GLCamera');
      return undefined;
    }

    return new EchoScanProgram(gl, glProgram, glCamera);
  }
  ...
  draw(object3D: Object3D<EchoScanMaterial>): void {
    const gl = this.gl;
    const program = this.glProgram.program;
    gl.useProgram(program);
